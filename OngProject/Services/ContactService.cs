...
namespace OngProject.Services
{
    public class ContactService : IContactService
    {
        ...
        {
            var contact =await _unitOfWork.ContactRepository.Add(con);
            return contact;
        }
        public async Task<Contact> Delete(int id)
        {
            var contact =await _unitOfWork.ContactRepository.Delete(id);
            return contact;
        }
        public async Task<Contact> Update(Contact con)
        {
            var contact =await _unitOfWork.ContactRepository.Update(con);
            return contact;
        }
    }
}
